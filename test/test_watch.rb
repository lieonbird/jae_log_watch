...
class TestWatch < Test::Unit::TestCase
  ...
  #
  #  assert_equal(db.calculate_req_between(app_id,t1.to_i,t3.to_i),2)
  #  assert_equal(db.calculate_req_between(app_id,t1.to_i,t3.to_i+1),3)
  #  assert_equal(db.calculate_req_between(app_id,t1.to_i,t4.to_i+1),-1)
  #
  ...
  def test_loadsavereadpos
    assert_nothing_raised do
      ...
      loadreadpos
      ino,pos = $ino, $pos
      $ino, $pos = 1000, 1234567
      savereadpos

      assert_equal($ino,1000)
      assert_equal($pos,1234567)
      $ino, $pos = ino, pos
      savereadpos
  ...
  def test_getlinedata
    line1 = %q{cf-test.jcloud.com - [12/Mar/2013:14:00:45 +0800] "GET /loadbalance/testcase HTTP/1.1" 500 955 196 "-" "Mozilla/5.0" 127.0.0.1 response_time:0.005 backend_addr:10.12.121.19:38412 load:0 app_id:314 }
    line2 = %q{cf-test.jcloud.com - [12/Mar/2013:14:00:45 +0800] "GET /loadbalance/testcase HTTP/1.1" 500 955 196 "-" "Mozilla/5.0" 127.0.0.1 response_time:5.0 backend_addr:10.12.121.19:38412 load:0 app_id:314 }
    line3 = %q{cf-test.jcloud.com - [12/Mar/2013:14:00:45 +0800] "GET /loadbalance/testcase HTTP/1.1" 500 955 196 "-" "Mozilla/5.0" 127.0.0.1 response_time:- backend_addr:10.12.121.19:38412 load:0 app_id:314 }
    ...
    assert_equal(data1[:errno],500)
    assert_equal(data1[:bytes_sent],955)
    assert_equal(data1[:request_length],196)

    assert_equal(data2[:app_id],314)
    ...
    assert_nothing_raised do
      ...
      dbtool.writelog(data1)
      dbtool.writelog(data2)
      ...
      assert_equal(data['data'][0]['metrics']['JAE_ReqNum'],'3')
  ...
  def test_notifyjca
    assert_nothing_raised do
      init
      raise 'jca return fail' unless notify_jca('1000', '/var/vcap.local/router/nginx/app_1000_access.log') == '200'
    end
  end
